                            ...
                            <tbody class="table-border-bottom-0">
                                @foreach ($orders as $k => $v)
                                    ...
                                    <tr aria-expanded="false">
                                        ...
                                        <td><a href="{{ route('order.info', $v->order_id) }}"
                                                title="{{ $v->order_id }}">{{ $v->order_id }}</a></td>
                                        <td><a href="{{ $v->website->website_url }}" target="_blank"
                                                title="Web Site Link ({{ $v->website->website_url }})"
                                                data-bs-toggle="tooltip" data-bs-placement="top"
                                                data-bs-custom-class="tooltip-secondary"
                                                data-bs-original-title="{{ $v->website->website_url }}">{{ $v->website->website_url }}</a>
                                        </td>

                                        <td>${{ $v->price }}</td>
                                        <td>{{ $v->attachment_type == 'provide_content' ? 'Blog Post' : 'Link Insertion' }}
                                        </td>
                                        <td>{{ $v->quantity }}</td>
                                        <td>
                                            {{ $v->payment_status == 'pending' ? 'Failed' : ucwords($v->payment_status) }}
                                            {{-- {{ $v->payment_status == 'pending' ? 'Failed' : ucwords($v->payment_status) }}{{ $v->has_complaint ? ' (Complaint)' : '' }} --}}
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                @if ($v->status == 'complete' && $v->advertiser_status != 'complete')
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        <li class="dropdown-item orderStatus{{ $v->id }} {{ $v->advertiser_status == 'new' ? 'active' : '' }}"
                                                            onclick="updateOrderStatus({{ $v->id }}, 'new')">New
                                                        </li>
                                                        <li class="dropdown-item orderStatus{{ $v->id }} {{ $v->advertiser_status == 'complete' ? 'active' : '' }}"
                                                            onclick="updateOrderStatus({{ $v->id }}, 'complete')">
                                                            Complete</li>
                                                        <li class="dropdown-item orderStatus{{ $v->id }} {{ $v->advertiser_status == 'update' ? 'active' : '' }}"
                                                            onclick="showChangeModal({{ $v->id }})">Update</li>
                                                    </ul>
                                                @else
                                                    <button type="button"
                                                        class="btn btn-label-primary waves-effect statusBtnTitle{{ $v->id }}">
                                                        {{ ucwords($v->advertiser_status) }}
                                                    </button>
                                                @endif
                                            </div>
