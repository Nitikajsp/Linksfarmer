                            ...
                            <table class="table dataTable table-responsive web_tab" id="websiteTbl">
                                ...
                                <tbody class="table-border-bottom-0">
                                    @foreach ($websites as $k => $v)
                                        ...
                                        @endphp
                                        <tr class="table-body" data-bs-toggle="collapse"
                                            data-bs-target="#website-{{ $k }}"
                                            ...
                                            <td class="p-0 text-center">
                                                @if ($v->site_verification_file != '')
                                                    {{-- <a href="{{ url('/storage/app/' . $v->site_verification_file) }}" target="_blank"><i class="tf-icons ti ti-clock-cog"></i></a> --}}
                                                        <a href="{{ route('google.download', ['filename' => urlencode(basename($v->site_verification_file))]) }}"
                                                            target="_blank">
                                                            <i class="tf-icons ti ti-clock-cog"></i>
                                                        </a>
                                                @else
                                            ...
                                            <td>
                                                <button type="button" class="btn p-0 edit-btn text-info"
                                                    onclick="window.location.href=`{{ url('/publisher/website') }}/{{ $v->id }}/edit`"><i
                                                        class="ti ti-pencil me-1"></i></button>
                                                <button type="button" class="btn p-0 delete-btn text-danger"
                                                    data-bs-toggle="modal" data-bs-target="#delete-pop"
                                                    onclick="$('.delete-yes-btn').attr('data-href',`{{ url('/publisher/website') }}/{{ $v->id }}/delete`);"><i
                                        ...
                                        <tr id="website-{{ $k }}" class="accordion-collapse collapse"
                                            ...
                                            <td class="accordion-body" colspan="8">
                                                <div id="historyCollapse"
                                                    class="accordion-collapse collapse show category-table col-12"
                                                    aria-labelledby="historyHeading" data-bs-parent="#historyAccordion">
                                                    <div class="row">
                                                        <div class="col-md-3 my-2"><strong>Role :</strong>
                                                            {{ $v->website_url }}</div>
                                                        <div class="col-md-3 my-2"><strong>Spam Score :</strong>
                                                            {{ $v->spam_score }}</div>
                                                        <div class="col-md-3 my-2"><strong>Min Word Count
                                                                :</strong>{{ $v->minimum_word_count_required }}</div>
                                                        <div class="col-md-3 my-2"><strong>Backlink :</strong>
                                                            {{ $v->backlink_type }}</div>
                                                        <div class="col-md-3 my-2"><strong>TAT:</strong>
                                                            {{ $v->publishing_time }} Days</div>
                                                        <!-- <div class="col-md-3 my-2"><strong>FC Guest Post Price :</strong> ${{ $v->fc_guest_post_price }}</div> -->
                                                        <div class="col-md-3 my-2"><strong>Link Insertion Price
                                                                :</strong>${{ $v->link_insertion_price }}</div>
                                                        <!-- <div class="col-md-3 my-2"><strong>FC Link Insertion Price :</strong>${{ $v->fc_link_insertion_price }}</div> -->
                                                        <div class="col-md-3 my-2"><strong>Maximum no.of backlinks
                                                                :</strong>{{ $v->maximum_no_of_backlinks_allowed }}</div>
                                                        <div class="col-md-3 my-2"><span
                                                                class="border-none badge bg-label-info me-1 p-2"
                                                                data-bs-toggle="tooltip" data-bs-placement="top"
                                                                data-bs-custom-class="tooltip-secondary"
                                                                data-bs-original-title="{!! count($forbidden_categories) >= 1 ? implode(', ', $forbidden_categories) : $forbidden_categories[0] !!}"
                                                                tooltip-position="right"> Forbidden Categories</span></div>
                                                        <div class="col-md-3 my-2"><span
                                                                class="border-none badge bg-label-info me-1 p-2"
                                                                data-bs-toggle="tooltip" data-bs-placement="top"
                                                                data-bs-custom-class="tooltip-secondary"
                                                                data-bs-original-title="{{ count($categories) >= 1 ? implode(', ', $categories) : $categories[0] }}"
                                                                tooltip-position="right">Categories</span></div>
                                                        <div class="col-md-3 my-2"><button type="button"
                                                                class="btn btn-dark" data-bs-toggle="modal"
                                                                data-bs-target="#guidelines-{{ $k }}">Guidelines</button>
                                                        </div>
                                                        <div class="col-md-3 my-2"><a href="{{ $v->sample_post_url }}"
                                                                target="_blank">View Sample</a></div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="modal fade" id="guidelines-{{ $k }}"
                    ...
                    <div class="modal-body">
                        ...
                        <ul style="line-height: 2;">
                            <li>
                                ...
                                <ul>
                                    <li>
                                        <p>{{ __('A minimum of 50 articles of a website should be indexed on Google. Link Publishers only considers websites with REAL high-quality traffic. PBN websites are immediately rejected from our list of accepted websites.') }}
                                        </p>
                                    ...
                                    <li>{{ __('Website Ahrefs or Semrush Organic Traffic should be greater than 250.') }}
                                    </li>
                                    <li>{{ __('MOZ Spam Score of a website can be no more than 15%.') }}</li>
                                </ul>
                            </li>
                            <li><strong>{{ __('Content on websites must be unique, readable and frequently updated.') }}</strong>
                            </li>
                            <li><strong>{{ __('Websites violating any laws are banned.') }}</strong></li>
                            <li><strong>{{ __('Websites not conforming to the ethical standards and highest moral is prohibited.') }}</strong>
                            </li>
                            <li><strong>{{ __('Forbidden categories that are accepted: CBD, Casino, Vape ,Cryptocurrency') }}</strong>
                            </li>
                            <li><strong>{{ __("We are also authorized to assess the website from an end-user's perspective. Thus, we may reject a website if our expectations are not met.") }}</strong>
                            </li>
                        </ul>
                        <div class="col-12 text-center">
                            <button type="reset" class="btn btn-primary me-sm-3 me-1 waves-effect waves-light agree-btn"
                                data-bs-dismiss="modal" aria-label="Close">{{ __('Agree & Continue') }}</button>
                        </div>
                    </div>
                </div>
            </div>
