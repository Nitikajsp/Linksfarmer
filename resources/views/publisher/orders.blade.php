                    ...
                    <table class="datatables-basic table table-responsive" id="orderTbl">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            <tr>
                               ...
                               <td>
    <button type="button" class="btn btn-label-primary dropdown-toggle waves-effect statusBtnTitle{{ $v->id }}" 
        data-bs-toggle="dropdown" aria-expanded="false">
        {{ ucwords($v->status) }}
    </button>
    <ul class="dropdown-menu">
        <li class="dropdown-item orderStatus{{ $v->id }} {{ $v->status == 'new' ? 'active' : '' }}" 
            onclick="orderStatus($(this), {{ $v->id }})" data-item="new">New</li>
        <li class="dropdown-item orderStatus{{ $v->id }} {{ $v->status == 'in-progress' ? 'active' : '' }}" 
            onclick="orderStatus($(this), {{ $v->id }})" data-item="in-progress">In-Progress</li>
        <li class="dropdown-item orderStatus{{ $v->id }} {{ $v->status == 'complete' ? 'active' : '' }}" 
            onclick="orderStatus($(this), {{ $v->id }})" data-item="complete">Complete</li>
        <li class="dropdown-item orderStatus{{ $v->id }} {{ $v->status == 'rejected' ? 'active' : '' }}" 
            onclick="orderStatus($(this), {{ $v->id }})" data-item="rejected">Rejected</li>
    </ul>
</td>
    ...
    if ($status === 'rejected') {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.error) {
                    $('#alert').removeClass().addClass('alert alert-danger')
                        .html('<ul class="m-auto"><li>' + response.error + '</li></ul>');
                } else {
                    $('#alert').removeClass().addClass('alert alert-success')
                        .html('<ul class="m-auto"><li>' + response.success + '</li></ul>');
            ...
            error: function(error) {
                ...
                $('#alert').removeClass().addClass('alert alert-danger')
                    .html('<ul class="m-auto"><li>' + errorMsg + '</li></ul>');
            }
        });
    }
}
