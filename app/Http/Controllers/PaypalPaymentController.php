...

{
    ...
    public function createPayment($price, $orderId)
    {
        ...
        if (!$token || isset($token['error'])) {
            \Log::error('PayPal access token failed:', (array) $token);
            return response()->json(['error' => 'Access token failed', 'details' => $token], 500);
        }
        ...
        return response()->json(['error' => 'Payment creation failed.', 'details' => $response], 500);
    }
                ...
                if ($customId) {
                    ...
                    if ($order) {
                        ...
                        if ($website) {
                            ...
                            $publisher = lslbUser::find($website->user_id); // Publisher
                            $buyer     = lslbUser::find($order->u_id);      // Buyer

                            // Get buyer info from PayPal
                            $paypalBuyerName  = $response['payer']['name']['given_name'] . ' ' . $response['payer']['name']['surname'];
                            $paypalBuyerEmail = $response['payer']['email_address'];
                            ...
                            $customData = [
                                ...
                                'payment_id'      => $captures['id'],
                                'categories'      => $categories,
                                'buyer_name'      => $paypalBuyerName,
                                'buyer_email'     => $paypalBuyerEmail,
                            ];

                            // Send email to Buyer
                            if ($buyer && $buyer->email) {
                                Mail::send('email.order_payment_successful_paypal', $customData, function ($message) use ($customData, $buyer) {
                                    $message->from($customData['mailaddress'], $customData['from_name']);
                                    $message->to($buyer->email);
                                    $message->subject($customData['subject']);
                                });
                            } else {
                                \Log::warning("Buyer user not found or email missing. Order ID: " . $order->id);
                            }

                            // Send email to Publisher
                            if ($publisher && $publisher->email) {
                                Mail::send('email.order_payment_successful_paypal_publisher', $customData, function ($message) use ($customData, $publisher) {
                                    $message->from($customData['mailaddress'], $customData['from_name']);
                                    $message->to($publisher->email);
                                    $message->subject('New orders have been successfully placed for your website');
                                });
                            } else {
                                \Log::warning("Publisher user not found or email missing. Website ID: " . $website->id);
                            }

                            // Remove website from cart
                            ...
                            \Log::error("Website not found for ID: " . $order->website_id);
                        }
                    } else {
                        \Log::error("Order not found for custom_id: " . $customId);
                    }
                }
